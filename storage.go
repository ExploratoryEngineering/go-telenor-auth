...
// Storage is the session and nonce storage used by the go-telenor-auth client.
...
// Session holds the session information from the Telenor Apigee server.
type Session struct {
...
func newSession(accessToken string, refreshToken string, expires int) Session {
...
func (m *memoryStorage) refreshAccessToken(config ClientConfig, session Session) {
	...
	refreshURL := buildApigeeURL(config, apigeeTokenPath)
